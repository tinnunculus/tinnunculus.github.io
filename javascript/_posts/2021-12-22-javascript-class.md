...
> * 클래스 필드를 이용해서 화살표 함수를 만들어주면 그 안에서 this는 항상 객체로 바인딩된다. (일반함수면 안된다) 이유는 모르겠다. 어쩌면 클래스 필드의 고유한 특징일지도....?
> ~~~js
> class Button{
>     constructor(value){
>         this.value = value;
...
> * 자바스크립트에서는 상속한 클래스의 constructor 함수에서는 특수 내부 property인 [[constructor]]: "derived" 가 존재한다.
...
> * 상속 클래스의 constructor가 new와 함께 실행되면, 빈 객체가 만들어지고 this에는 아무런 객체를 할당하지 않는다. 그래서 오류가 걸린다.
...
> * static method는 클래스 함수 자체 메소드이고 class field는 객체의 property이다. 헷갈리지 말자.
> ~~~js
> class Article{
...
> * static method와 static property는 일반 mothod와 property처럼 상속이 가능하다.
> ~~~js
> class Animal{
>     static planet = "지구";
> }
...
## 객체를 상속하는 클래스
> * 클래스는 실제로는 함수(객체)이기 때문에 함수(객체)를 상속할 수도 있다.
...
> console.log(rabbit.hasOwnProperty('name')); // true,, Rabbit 클래스는 어떠한  클래스도 상속하지 않는 일반 클래스인데(derived = false) 어떻게 hasOwnProperty를 쓸 수 있던 것일까?
...
> }
> let rabbit = new Rabbit("Rab");
> console.log(rabbit.hasOwnProperty('name')); // true,, Object() constructor가 실행됨으로써 hasOwnProperty를 사용할 수 있다.
> console.log(Rabbit.prototype.__proto__ === Object.prototype); // true
...
> * 디폴트로 사용하는 것이 public이고 이름 앞에 #을 붙이면 private로 설정해서 클래스 내에서만 접근할 수 있도록 한다.
...
> * 아래의 코드에서 내장 클래스로부터 상속받은 메소드(filter, map)같은 것을 사용할 때 Array 객체가 아닌 PowerArray 객체를 리턴한다.
...
> * 클래스의 [[prototype]] property가 상속하는 클래스를 가리키지 못하기 때문이다. 일반 클래스의 상속의 경우 [[prototype]] property는 상속하는 클래스를 가리킨다.
